...
export class AuthService {
  ...
  async funRegister(objUser: RegisterAuthDto) {
    ...
    const existingUser = await this.userRepository.findOne({ where: { email: objUser.email } });
    if (existingUser) {
      throw new HttpException('El correo electrónico ya está registrado', 400);
    }
    ...
    const newUser = await this.userRepository.save(objUser);
    
    // Devuelve el usuario junto con un mensaje de éxito
    return {
      success: true,
  ...
  async forgotPassword(email: string) {
    ...
    const resetLink = `${process.env.FRONTEND_BASE_URL}/auth/reset-password?token=${token}`;
